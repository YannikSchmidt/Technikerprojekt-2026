...
ModbusRTU mb;
const int SLAVE_ID = 1;       // =gakID (Kann nicht 0 sein, da 0 Broadcast ist. Maximal 247)
...
const float SENSOR_SENSITIVITY = 0.020;  //80 mV/A (CYHCS-LSP25) // 20 mV/A (ACS758-100B)
...
void calculateAndPublish() {
  ...
  for (int i = 0; i < 8; i++) {
      // Durch VALID_SAMPLES teilen, da wir die extremen Werte verworfen haben
...
//Auswertung und Modbus-Publishing
    calculateAndPublish();
  }
}