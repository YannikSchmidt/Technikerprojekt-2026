...
const uint8_t SENSOR_PINS[CHANNELS]  = { 4, 25, 36, 39, 34, 35, 32, 33 };
const char*   SENSOR_NAMES[CHANNELS] = { "U1", "U2", "U3", "U4", "U5", "U6", "U7", "U8" };
...
// Unterdrückt Rauschen um den Nullpunkt.
const float DEADBAND_AMPERE = 0.5;
...
void readSensors() {
  ...
  for (int i = 0; i < CHANNELS; i++) {
    ...
    if (amps > -DEADBAND_AMPERE && amps < DEADBAND_AMPERE) amps = 0.0f;
...
// Bildet aus den gesammelten Punktmessungen den Mittelwert, schreibt ihn in
// die Modbus-Register und erhöht die Paketnummer.
void calculateAndPublishAverage() {
  Serial.printf("\n---[ GAK Nr:%u | Paket: %u ] MITTELWERT ---\n", SLAVE_ID, packetNr);

  mb.Hreg(MODBUS_START_REG + REG_PACKET, packetNr);
  ...
  for (int i = 0; i < CHANNELS; i++) {
    ...
    mb.Hreg(MODBUS_START_REG + REG_VALUES + i, (uint16_t)milliampere);

    Serial.printf("Strang %d (%s): %.2f A (Modbus: %d mA)\n", i + 1, SENSOR_NAMES[i], avg_amps, milliampere);
  }

  packetNr++;   // uint16_t springt nach 65535 von selbst auf 0
}
...
// --- setup() -------------------------------------------------------------
// Wird einmal beim Einschalten ausgeführt: startet RS485, meldet den Slave
// an und legt die Modbus-Register an.
void setup() {
  Serial.begin(115200);
  delay(500);
  ...
  Serial.printf("Modbus Slave ID %u, Register %u..%u\n", SLAVE_ID, MODBUS_START_REG, MODBUS_START_REG + MODBUS_REG_COUNT - 1);
  Serial.println("Setup abgeschlossen. Starte Punktmessung...\n");
}

// --- loop() --------------------------------------------------------------
// Läuft endlos: bedient Modbus und startet alle 3 s eine Punktmessung.
void loop() {
  ...
  if (currentSampleCount >= SAMPLES_PER_AVERAGE) {
    ...
    for (int i = 0; i < CHANNELS; i++) currentSum[i] = 0;
  }
}
